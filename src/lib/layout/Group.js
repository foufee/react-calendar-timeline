...
export default class Group extends Component {
  ...
  cacheDataFromProps (props) {
    this.groupId = _get(props.group, props.keys.groupIdKey)
  ...
  componentWillReceiveProps (nextProps) {
    ...
    if (!interactMounted) {
      this.mountInteract()
      this.state.interactMounted = true
    }
  }
  ...
  handleContextMenu = (e) => {
    if (this.props.onContextMenu) {
      e.preventDefault()
      e.stopPropagation()
      this.props.onContextMenu(this.groupId, e)
    }
  };

  actualClick (e, clickType) {
    if (this.props.canSelect && this.props.onSelect) {
      this.props.onSelect(this.groupId, clickType, e)
    }
  }
